...
export default function App() {
  ...
  const handleSaveProduct = () => {
    ...
    const newProductData = { 
      ...
      enrollDate: newProduct.enrollDate,
      author: userProfile.name || '본인'
    };
    const existingProducts = selectedCustomer.products || [];
    updateSelectedCustomer({ products: [newProductData, ...existingProducts] });
    setIsAddingProduct(false);
    showToast("가입 상품 내역이 추가되었습니다.");
  };

  const handleAddHashtag = () => {
    if (hashtagInput.trim()) {
      const currentTags = selectedCustomer.hashtags || [];
      if (!currentTags.includes(hashtagInput.trim())) {
        updateSelectedCustomer({ hashtags: [...currentTags, hashtagInput.trim()] });
      }
    }
    setHashtagInput('');
  ...
  const handleRemoveHashtag = (tag) => {
    const currentTags = selectedCustomer.hashtags || [];
    updateSelectedCustomer({ hashtags: currentTags.filter(t => t !== tag) });
  };
  ...
  const handleSaveHistory = () => {
    ...
    const latestDate = allHistory[0].date;

    updateSelectedCustomer({ history: allHistory, lastContact: latestDate, note: allHistory[0].note });
    setIsAddingHistory(false);
    setNewHistory({ type: '통화', date: getOffsetDate(0), time: '', location: '', alarm: '알림 없음', note: '' });
    showToast("새로운 상담 이력이 등록되었습니다.");
  };
  ...
  const handleSaveMemo = () => {
    ...
    setIsAddingMemo(false);
    setNewMemoText('');
    showToast("새로운 메모가 등록되었습니다.");
  };

  const filteredCustomers = useMemo(() => {
    let result = customers;
    if (searchQuery) {
        result = result.filter(c => c.name.includes(searchQuery) || c.company.includes(searchQuery) || (c.product && c.product.includes(searchQuery)));
    }
    if (customerTab === '공유') {
      return result.filter(c => c.isSharedWithMe).sort((a, b) => new Date(b.lastContact) - new Date(a.lastContact));
    }
  ...
  const startMainScan = () => {
    ...
    setTimeout(() => {
      setScannedData({
        name: "정우성", company: "미래에셋증권", position: "지점장", phone: "010-3333-4444", email: "ws.jung@miraeasset.com", status: "가망", note: "명함 스캔으로 신규 등록됨"
      });
  ...
  const handleSaveScannedData = async () => {
    ...
    try {
      ...
      setIsMainScanning(false);
      setScannedData(null);
      showToast(`${newCustomer.name} 고객이 ${newCustomer.status} 상태로 등록되었습니다.`);
      setActiveTab('customers');
      setCustomerTab(newCustomer.status);
  ...
  const handleSaveManualData = async () => {
    ...
    try {
      ...
      setIsMainScanning(false);
      setManualData({ name: '', phone: '', job: '회사원', company: '', dept: '', position: '' });
      showToast(`${newCustomer.name} 고객이 가망 상태로 등록되었습니다.`);
      setActiveTab('customers');
      setCustomerTab('가망');
    } catch(e) { console.error(e); }
  };
  ...
  const toggleHistoryExpand = (id) => {
    setExpandedHistoryIds(prev => prev.includes(id) ? prev.filter(item => item !== id) : [...prev, id]);
  };
  ...
  const startUpdateScan = () => {
    ...
    setTimeout(() => {
      setScannedData({
        name: selectedCustomer.name, company: "(주)새로운직장", position: "임원", phone: selectedCustomer.phone, email: selectedCustomer.email || "new@email.com", status: selectedCustomer.status, note: "이직으로 인한 명함 업데이트"
      });
  ...
  const handleSaveUpdatedCard = () => {
    ...
    setIsMainScanning(false);
    setScannedData(null);
    showToast(`${updatedCustomer.name} 고객님의 명함이 업데이트 되었습니다.`);
  };

  const handleAddNewProfileCard = () => {
    setIsMyCardsModalOpen(false);
    setUserProfile({ id: String(Date.now()), name: '', company: '', dept1: '', dept2: '', position: '', phone: '', email: '', experience: '', certs: [], isPublic: true });
    setAppState('scan');
  };
  ...
  const handleCompleteProfileStep2 = async () => {
    ...
    try {
      ...
      setAppState('main');
      showToast("프로필 명함이 성공적으로 등록되었습니다.");
    } catch(e) { console.error(e); }
  };
  ...
  const handleCompleteGeneralCard = async () => {
    ...
    try {
      ...
      setUserProfile(newCard); 
      setAppState('main');
      showToast("일반 명함이 성공적으로 등록되었습니다.");
    } catch(e) { console.error(e); }
  };

  const handleNotiClick = (noti) => {
    if (noti.type === '공지') {
      setPopupData({ type: 'notice', data: noti });
    } else if (noti.type === '약속') {
      setPopupData({ type: 'appointment', data: noti });
    }
  };
  ...
  const handleAcceptShare = async (e, noti) => {
    ...
    showToast(`${newCustomer.name} 고객님 공유를 수락하여 리스트에 추가했습니다.`);
  };

  const handleRejectShare = (e, id) => {
    e.stopPropagation();
    setNotifications(notifications.filter(n => n.id !== id));
    showToast("고객 공유 요청을 거절했습니다.");
  };
  ...
  const renderIntroPage = () => (
    <div className="absolute inset-0 z-[200] bg-white flex flex-col items-center justify-center animate-in fade-in duration-700">
      <div className="flex flex-col items-center gap-6 animate-in zoom-in-95 duration-1000">
        ...
        <div className="text-center space-y-2"><h1 className="text-4xl font-black text-slate-900 tracking-tighter">ReadUp <span className="text-indigo-600">Pro</span></h1><p className="text-xs font-bold text-slate-400 tracking-widest uppercase">Insurance Sales Master</p></div>
      </div>
  ...
  const renderLoginPage = () => (
    <div className="absolute inset-0 z-[190] bg-white flex flex-col px-8 pt-24 pb-12 animate-in slide-in-from-right duration-300">
      <div className="flex-1">
        ...
        <p className="text-sm font-bold text-slate-400 mt-4 leading-relaxed">고객 관리부터 팀 협업까지<br/>보험 영업 프로를 위한 완벽한 솔루션</p>
      </div>
  ...
  const renderTermsPage = () => {
    ...
    return (
      <div className="absolute inset-0 z-[180] bg-slate-50 flex flex-col px-6 pt-20 pb-10 animate-in slide-in-from-right duration-300">
        ...
        <button disabled={!canProceed} onClick={() => setAppState('scan')} className={`w-full py-5 rounded-2xl font-black text-base transition-all mt-6 ${canProceed ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200 active:scale-95' : 'bg-slate-200 text-slate-400'}`}>동의하고 다음으로</button>
      </div>
    );
  };
  ...
  const renderProfileStep1Page = () => (
    <div className="absolute inset-0 z-[160] bg-slate-50 flex flex-col animate-in slide-in-from-right duration-300">
      <div className="bg-white px-6 pt-14 pb-5 border-b border-slate-100 sticky top-0 z-10 shadow-sm flex items-center justify-between"><div><h2 className="text-2xl font-black text-slate-900">프로필 등록 <span className="text-indigo-600 ml-1">1/2</span></h2><p className="text-[11px] font-bold text-slate-400 mt-1">스캔된 명함 정보를 확인하고 수정하세요.</p></div></div>
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 pb-32 hide-scrollbar">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><label className="text-[10px] font-bold text-slate-400 ml-1">소속 회사</label><input type="text" value={userProfile.company} onChange={e=>setUserProfile({...userProfile, company:e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mt-1 outline-none focus:border-indigo-400" /></div>
            <div><label className="text-[10px] font-bold text-slate-400 ml-1">소속 본부 (선택)</label><input type="text" value={userProfile.dept1} placeholder="예: 수도권본부" onChange={e=>setUserProfile({...userProfile, dept1:e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mt-1 outline-none focus:border-indigo-400" /></div>
            <div><label className="text-[10px] font-bold text-slate-400 ml-1">소속 팀 (선택)</label><input type="text" value={userProfile.dept2} placeholder="예: 영업1팀" onChange={e=>setUserProfile({...userProfile, dept2:e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mt-1 outline-none focus:border-indigo-400" /></div>
            <div><label className="text-[10px] font-bold text-slate-400 ml-1">직급</label><input type="text" value={userProfile.position} onChange={e=>setUserProfile({...userProfile, position:e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mt-1 outline-none focus:border-indigo-400" /></div>
            <div><label className="text-[10px] font-bold text-slate-400 ml-1">이름</label><input type="text" value={userProfile.name} onChange={e=>setUserProfile({...userProfile, name:e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mt-1 outline-none focus:border-indigo-400" /></div>
            <div className="col-span-2"><label className="text-[10px] font-bold text-slate-400 ml-1">연락처</label><input type="text" value={userProfile.phone} onChange={e=>setUserProfile({...userProfile, phone:e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mt-1 outline-none focus:border-indigo-400" /></div>
          </div>
        </div>
      </div>
      <div className="absolute bottom-0 inset-x-0 p-6 bg-gradient-to-t from-slate-50 via-slate-50 to-transparent"><button onClick={() => myBusinessCards.length === 0 ? setAppState('profile_step2') : handleCompleteGeneralCard()} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-base shadow-xl shadow-indigo-200 active:scale-95 transition-all">{myBusinessCards.length === 0 ? "다음 단계로" : "프로필 등록 완료"}</button></div>
    </div>
  );
  ...
  const renderProfileStep2Page = () => {
    ...
    return (
      <div className="absolute inset-0 z-[160] bg-slate-50 flex flex-col animate-in slide-in-from-right duration-300">
        <div className="bg-white px-6 pt-14 pb-5 border-b border-slate-100 sticky top-0 z-10 shadow-sm flex items-center gap-3"><button onClick={() => setAppState('profile_step1')} className="w-8 h-8 flex items-center justify-center bg-slate-50 rounded-full text-slate-600 active:scale-95"><ChevronLeft size={18} className="pr-0.5" /></button><div><h2 className="text-2xl font-black text-slate-900">프로필 등록 <span className="text-indigo-600 ml-1">2/2</span></h2><p className="text-[11px] font-bold text-slate-400 mt-1">전문성을 나타낼 영업 정보를 선택해주세요.</p></div></div>
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8 pb-32 hide-scrollbar">
          <div className="space-y-6">
            <div>
              <label className="text-[10px] font-black text-slate-400 ml-1 mb-2 block uppercase tracking-widest">보유 자격 리스트 (필수)</label>
              ...
              <div className="flex flex-wrap gap-2">
                {CERTIFICATIONS.map(cert => {
                  ...
                  return (
                    <button key={cert} type="button" onClick={() => toggleCert(cert)} className={`px-4 py-2.5 rounded-xl text-xs font-black transition-all border ${isActive ? 'bg-indigo-600 border-indigo-600 text-white shadow-sm' : 'bg-white border-slate-200 text-slate-600 hover:bg-slate-50'}`}>
                      {cert}
                    </button>
                  )
                })}
    ...
    return (
      <div className="p-5 space-y-5 animate-in fade-in duration-500 pb-32 flex flex-col min-h-full">
        ...
        <section className="bg-white rounded-xl border border-slate-100 p-4 shadow-sm flex-1">
           <h3 className="text-xs font-black text-slate-900 mb-3">최근 관리된 연락처</h3>
           <div className="space-y-3">
    ...
    <div className="p-5 space-y-4 animate-in slide-in-from-right duration-300 pb-32">
      ...
      <div className="grid grid-cols-1 gap-3">
        {filteredCustomers.length === 0 ? (
          <div className="py-16 text-center text-slate-400 font-bold text-sm">해당 분류에 등록된 고객이 없습니다.</div>
        ) : (
          filteredCustomers.map((c, index) => (
            <React.Fragment key={c.id}>
              {index === 2 && <GoogleAdPlaceholder type="infeed" className="mb-3" />}
              
              <div onClick={() => { setSelectedCustomer(c); setCustomerDetailTab('기본정보'); }} className="bg-white rounded-2xl p-5 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] border border-slate-100 flex items-center justify-between group cursor-pointer hover:border-indigo-200 active:bg-slate-50 transition-all">
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-12 h-12 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-400 text-lg font-black group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-colors shrink-0">{c.name[0]}</div>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {noti.type === '공유' && noti.isNew && (
                      <div className="flex gap-2 mt-3 pt-3 border-t border-slate-100">
                        ...
                        <button onClick={(e) => handleAcceptShare(e, noti)} className="flex-1 py-2 bg-indigo-600 text-white text-[11px] font-black rounded-lg active:scale-95 transition-all shadow-sm shadow-indigo-200">수락하기</button>
                      </div>
                    )}
                  </div>
                  {noti.isNew && <div className="w-2 h-2 rounded-full bg-rose-500 mt-2"></div>}
                </div>
              );
            })
          )}
           ...
           {displayMembers.map(member => (
             <div key={member.id} onClick={() => setSelectedTeamMember(member)} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-[0_2px_10px_-3px_rgba(0,0,0,0.05)] flex items-center justify-between hover:border-indigo-300 transition-all group cursor-pointer active:scale-[0.98]">
               ...
               <div className="flex gap-2">
                 <button onClick={(e) => { e.stopPropagation(); showToast('메시지를 작성합니다.'); }} className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors shadow-sm"><MessageCircle size={14} /></button>
                 <button onClick={(e) => { e.stopPropagation(); showToast('전화 연결합니다.'); }} className="w-9 h-9 rounded-full bg-slate-50 text-slate-400 flex items-center justify-center hover:bg-indigo-50 hover:text-indigo-600 transition-colors shadow-sm"><Phone size={14} /></button>
               </div>
             </div>
           ))}
        </div>
      ...
      <div className="pb-10 animate-in slide-in-from-right duration-300 bg-[#f8fafc] min-h-full">
          ...
          <div className="relative">
            ...
            <div className={`rounded-[2rem] p-6 shadow-md border relative overflow-hidden aspect-[1.6/1] flex flex-col justify-between z-10 transition-all ${
              ...
              <div className="space-y-1.5 relative z-10 mt-3">
                ...
                {displayCard.email && <p className={`text-xs font-bold flex items-center gap-2.5 tracking-wide ${selectedCustomer.isSharedWithMe ? 'text-purple-700' : 'text-slate-600'}`}><Mail size={14} className={selectedCustomer.isSharedWithMe ? 'text-purple-400' : 'text-slate-400'}/> {displayCard.email}</p>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="px-5 flex gap-2 mb-6">
          <button onClick={() => window.location.href=`tel:${selectedCustomer.phone}`} className="flex-1 bg-white border border-slate-200 text-slate-700 py-3.5 rounded-xl text-[11px] font-black flex items-center justify-center gap-1.5 shadow-sm hover:bg-slate-50 transition-all active:scale-95"><Phone size={14}/> 전화</button>
           ...
           <button onClick={() => showToast('카카오톡 채널로 연결합니다.')} className="flex-1 bg-[#FEE500] text-black py-3.5 rounded-xl text-[11px] font-black flex items-center justify-center gap-1.5 shadow-sm hover:bg-[#FDD800] transition-all active:scale-95"><MessageSquare size={14} className="fill-black"/> 카톡</button>
        </div>
        ...
        <div className="p-5 pb-10">
          {customerDetailTab === '기본정보' && (
             <div className="space-y-3 animate-in fade-in duration-300">
                 ...
                 {!isEditingStatus ? (
                    ...
                    <div className="space-y-2.5 animate-in fade-in duration-200">
                      ...
                      <div className="flex gap-2 pt-1">
                        ...
                        <button onClick={() => { updateSelectedCustomer({ status: tempStatus }); setIsEditingStatus(false); showToast("고객 상태가 변경되었습니다."); }} className="flex-[2] py-2 bg-indigo-600 text-white rounded-xl text-[11px] font-black shadow-md shadow-indigo-200 transition-transform active:scale-95">저장 확인</button>
                      </div>
                    </div>
                 )}
               </div>
               ...
               {!selectedCustomer.isSharedWithMe && (
                 <div className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm flex justify-between items-center cursor-pointer hover:border-indigo-200 transition-colors" onClick={() => { setShareSearchQuery(''); setSelectedShareMembers(selectedCustomer.sharedWith || []); setIsShareModalOpen(true); }}>
                   <div>
          ...
          {customerDetailTab === '상담이력' && (
             <div className="animate-in fade-in duration-300 space-y-4">
               {!isAddingHistory ? (
                                  ...
                                  <div className={`overflow-hidden transition-all ${isExpanded ? 'max-h-[500px]' : 'max-h-[38px]'}`}>
                                    {h.type === '미팅약속' && (h.time || h.location) && (
                                      <div className={`mb-2 pb-2 border-b border-slate-200/60 space-y-1.5 ${!isExpanded ? 'hidden' : ''}`}>
                                         {h.time && <p className="text-[10px] font-black text-rose-600 flex items-center gap-1.5"><Clock size={12}/> 약속 시간: {h.time} {h.alarm && h.alarm !== '알림 없음' ? <span className="bg-rose-100 px-1.5 py-0.5 rounded text-[8px]">🔔 {h.alarm}</span> : ''}</p>}
                 ...
                 <div className="bg-white p-5 rounded-2xl border border-indigo-100 shadow-lg space-y-5 animate-in slide-in-from-bottom-4 duration-300">
                   <h4 className="text-sm font-black text-slate-900 border-b border-slate-100 pb-3">신규 상담 이력 등록</h4>
                   <div className="space-y-4">
                     <div>
                       <label className="text-[10px] font-bold text-slate-400 mb-2 block">상담 일자</label>
                       <div className="flex gap-2 mb-2">
                         <button type="button" onClick={() => setNewHistory({...newHistory, date: getOffsetDate(-2)})} className="flex-1 py-1.5 bg-slate-50 text-slate-500 rounded-lg text-[10px] font-black hover:bg-slate-100">그제</button>
                         <button type="button" onClick={() => setNewHistory({...newHistory, date: getOffsetDate(-1)})} className="flex-1 py-1.5 bg-slate-50 text-slate-500 rounded-lg text-[10px] font-black hover:bg-slate-100">어제</button>
                         <button type="button" onClick={() => setNewHistory({...newHistory, date: getOffsetDate(0)})} className="flex-1 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg text-[10px] font-black border border-indigo-100">오늘</button>
                         <button type="button" onClick={() => setNewHistory({...newHistory, date: getOffsetDate(1)})} className="flex-1 py-1.5 bg-slate-50 text-slate-500 rounded-lg text-[10px] font-black hover:bg-slate-100">내일</button>
                       </div>
                       <input type="date" value={newHistory.date} onChange={e => setNewHistory({...newHistory, date: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-black text-slate-800 outline-none focus:border-indigo-400" />
                     </div>
                     <div>
                       <label className="text-[10px] font-bold text-slate-400 mb-2 block">상담 채널 / 방식</label>
                     ...
                     {newHistory.type === '미팅약속' && (
                       <div className="space-y-4 animate-in fade-in duration-300 border-t border-slate-100 pt-4 mt-4">
                          ...
                          <div><label className="text-[10px] font-bold text-slate-400 mb-1.5 block">미팅 장소 *</label><input type="text" placeholder="예: 강남역 3번출구 스타벅스" value={newHistory.location} onChange={e => setNewHistory({...newHistory, location: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 outline-none focus:border-rose-400" /></div>
                       </div>
                     )}
                     <div>
                       <label className="text-[10px] font-bold text-slate-400 mb-2 block">{newHistory.type === '미팅약속' ? '미팅 목적 / 특이사항' : '상담 내용'}</label>
                       <textarea rows="4" placeholder={newHistory.type === '미팅약속' ? "미팅의 목적이나 사전에 준비할 특이사항을 기록해 주세요." : "고객과의 상담 내용이나 특이사항을 상세히 기록해 주세요."} value={newHistory.note} onChange={e => setNewHistory({...newHistory, note: e.target.value})} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 outline-none focus:border-indigo-400 resize-none leading-relaxed"></textarea>
                     </div>
                   </div>
                   <div className="flex gap-2 pt-2">
                     <button onClick={() => setIsAddingHistory(false)} className="flex-1 bg-slate-100 text-slate-500 py-3.5 rounded-xl font-black text-xs transition-colors active:bg-slate-200">취소</button>
                     <button onClick={handleSaveHistory} className="flex-[2] bg-indigo-600 text-white py-3.5 rounded-xl font-black text-xs transition-transform active:scale-95 shadow-md shadow-indigo-200">이력 등록하기</button>
                   </div>
                 </div>
               )}
             </div>
             ...
             <div className="animate-in fade-in duration-300">
               {!isAddingProduct ? (
                 <div className="space-y-6">
                   ...
                   <div>
                     ...
                     {!(selectedCustomer.products?.filter(p => p.origin === '가입상품' || p.origin === '신규 계약').length > 0) ? (
                       <div className="bg-slate-50 p-4 rounded-xl text-center text-[10px] font-bold text-slate-400 border border-slate-100">등록된 가입 상품이 없습니다.</div>
                     ) : (
                       <div className="space-y-2">
                         {selectedCustomer.products.filter(p => p.origin === '가입상품' || p.origin === '신규 계약').map(p => {
                           ...
                           return (
                             <div key={p.id} className="bg-white p-4 rounded-2xl border border-emerald-100 shadow-sm relative overflow-hidden">
                               <div className="absolute top-0 left-0 w-1 h-full" style={{ backgroundColor: bgColor }}></div>
                               <div className="flex justify-between items-center mb-1.5 pl-2">
                                 <div className="flex items-center gap-1.5"><span className="text-[9px] font-black px-1.5 py-0.5 rounded" style={{ color: bgColor, backgroundColor: `${bgColor}15` }}>{p.company}</span><span className="text-[9px] font-bold text-slate-400 flex items-center gap-1"><span className="text-emerald-500 font-black">[{p.origin}]</span> {p.enrollDate} 가입</span></div>
                                 {p.author && <span className="text-[8px] font-black text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{p.author} 등록</span>}
                               </div>
                   ...
                   <div>
                     ...
                     {!(selectedCustomer.products?.filter(p => p.origin === '기존상품' || p.origin === '기존 가입').length > 0) ? (
                       <div className="bg-slate-50 p-4 rounded-xl text-center text-[10px] font-bold text-slate-400 border border-slate-100">등록된 기존 상품이 없습니다.</div>
                     ) : (
                       <div className="space-y-2">
                         {selectedCustomer.products.filter(p => p.origin === '기존상품' || p.origin === '기존 가입').map(p => {
                           ...
                           return (
                             <div key={p.id} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm relative overflow-hidden">
                               ...
                               <div className="flex justify-between items-center mb-1.5 pl-2">
                                 <div className="flex items-center gap-1.5"><span className="text-[9px] font-black px-1.5 py-0.5 rounded" style={{ color: bgColor, backgroundColor: `${bgColor}15` }}>{p.company}</span><span className="text-[9px] font-bold text-slate-400">{p.enrollDate} 가입</span></div>
                                 {p.author && <span className="text-[8px] font-black text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{p.author} 등록</span>}
                               </div>
                               <h5 className="text-sm font-black text-slate-700 tracking-tight pl-2">{p.name}</h5>
                               <p className="text-[10px] font-bold text-slate-400 mt-1.5 pl-2">가입금액: <span className="text-slate-600">{p.amount}</span></p>
                             </div>
                           )
                         })}
                       </div>
                   ...
                   <div>
                     ...
                     {!(selectedCustomer.products?.filter(p => p.origin === '소개상품').length > 0) ? (
                       <div className="bg-slate-50 p-4 rounded-xl text-center text-[10px] font-bold text-slate-400 border border-slate-100">등록된 추천/소개 상품이 없습니다.</div>
                     ) : (
                       <div className="space-y-2">
                         {selectedCustomer.products.filter(p => p.origin === '소개상품').map(p => {
                           ...
                           const bgColor = compData.color;
                           return (
                             <div key={p.id} className="bg-white p-4 rounded-2xl border border-blue-100 shadow-sm relative overflow-hidden">
                               <div className="absolute top-0 left-0 w-1 h-full" style={{ backgroundColor: bgColor }}></div>
                               <div className="flex justify-between items-center mb-1.5 pl-2">
                                 <div className="flex items-center gap-1.5"><span className="text-[9px] font-black px-1.5 py-0.5 rounded" style={{ color: bgColor, backgroundColor: `${bgColor}15` }}>{p.company}</span><span className="text-[9px] font-bold text-slate-400 flex items-center gap-1"><span className="text-blue-500 font-black">[{p.origin}]</span> {p.enrollDate} 추천</span></div>
                                 {p.author && <span className="text-[8px] font-black text-slate-500 bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">{p.author} 등록</span>}
                               </div>
                               <h5 className="text-sm font-black text-slate-800 tracking-tight pl-2">{p.name}</h5>
                   ...
                   <button onClick={() => setIsAddingProduct(true)} className="w-full py-4 mt-2 border-2 border-dashed border-indigo-200 bg-indigo-50/50 rounded-2xl text-indigo-500 font-black text-xs flex items-center justify-center gap-2 hover:border-indigo-400 hover:bg-indigo-100 transition-colors active:scale-95"><Plus size={16} /> 새로운 가입/소개상품 등록</button>
                 ...
                 <div className="bg-white p-5 rounded-2xl border border-indigo-100 shadow-lg space-y-5 animate-in slide-in-from-bottom-4 duration-300">
                   ...
                   <div className="space-y-5">
                     <div>
                       <label className="text-[10px] font-bold text-slate-400 mb-2 block uppercase tracking-widest">상품 출처 구분</label>
                       <div className="flex gap-2">
                         {['가입상품', '기존상품', '소개상품'].map(origin => (
                           <button key={origin} type="button" onClick={() => setNewProduct({...newProduct, origin})} className={`flex-1 py-2.5 rounded-xl text-[11px] font-black transition-all border ${newProduct.origin === origin ? 'border-indigo-500 bg-indigo-600 text-white shadow-md shadow-indigo-200' : 'border-slate-200 bg-slate-50 text-slate-500 hover:bg-slate-100'}`}>{origin}</button>
                         ))}
                       </div>
                     </div>
                     <div>
                       <label className="text-[10px] font-bold text-slate-400 mb-2 block uppercase tracking-widest">보험 종류</label>
                       <div className="flex gap-2">
                         {['생명보험', '손해보험'].map(cat => (
                           <button key={cat} type="button" onClick={() => setNewProduct({...newProduct, category: cat})} className={`flex-1 py-2.5 rounded-xl text-xs font-black border transition-all ${newProduct.category === cat ? 'border-indigo-500 text-indigo-600 bg-indigo-50' : 'border-slate-200 text-slate-500 bg-white'}`}>{cat}</button>
                         ))}
                       </div>
                     </div>
                     <div>
                       <label className="text-[10px] font-bold text-slate-400 block mb-1.5 uppercase tracking-widest">가입 보험사</label>
                       <div className="relative">
                         <select value={newProduct.company} onChange={(e) => setNewProduct({...newProduct, company: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl pl-4 pr-10 py-3.5 text-sm font-black outline-none focus:border-indigo-400 appearance-none shadow-sm">
                     ...
                     <div className="space-y-3 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                       ...
                       {newProduct.isCustomName && (
                         <div className="animate-in slide-in-from-top-2 duration-200">
                           <input type="text" placeholder="상품명을 직접 입력하세요" value={newProduct.customName} onChange={(e) => setNewProduct({...newProduct, customName: e.target.value})} className="w-full bg-white border border-indigo-300 rounded-xl px-4 py-3 text-sm font-bold text-indigo-900 outline-none focus:ring-2 focus:ring-indigo-100 placeholder-indigo-300/50 shadow-inner" />
                         </div>
                       )}
                     </div>
                     <div className="grid grid-cols-2 gap-3">
                       <div><label className="text-[10px] font-bold text-slate-400 block mb-1.5 uppercase tracking-widest">가입 금액</label><input type="text" placeholder="예: 1억원" value={newProduct.amount} onChange={(e) => setNewProduct({...newProduct, amount: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 text-sm font-bold outline-none focus:border-indigo-400 shadow-sm" /></div>
                   ...
                   <div className="flex gap-2 pt-4 border-t border-slate-100">
                     <button onClick={() => setIsAddingProduct(false)} className="flex-1 bg-slate-100 text-slate-500 py-3.5 rounded-xl font-black text-xs transition-colors active:bg-slate-200">취소</button>
                     <button onClick={handleSaveProduct} className="flex-[2] bg-indigo-600 text-white py-3.5 rounded-xl font-black text-xs transition-transform active:scale-95 shadow-md shadow-indigo-200 flex items-center justify-center gap-1.5"><CheckCircle2 size={16} /> 상품 등록하기</button>
                   </div>
                 </div>
               )}
             </div>
               ...
               {!isAddingMemo ? (
                 <>
                   <div className="flex justify-between items-center px-1">
                     ...
                     <button onClick={() => { setIsAddingMemo(true); setNewMemoText(''); }} className="text-[10px] font-bold text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg flex items-center gap-1 hover:bg-indigo-100 transition-colors shadow-sm"><Plus size={12} strokeWidth={3} /> 신규 메모 추가</button>
                   </div>
                   <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm min-h-[200px] flex flex-col">
                     <div className="flex-1 space-y-4">
                       {selectedCustomer.history && selectedCustomer.history.filter(h => h.type === '메모').length > 0 ? (
                         selectedCustomer.history.filter(h => h.type === '메모').map(h => {
                           const isExpanded = expandedHistoryIds.includes(h.id);
                           return (
                             <div key={h.id} onClick={() => toggleHistoryExpand(h.id)} className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 relative cursor-pointer hover:border-indigo-300 hover:shadow-sm transition-all">
                               <div className="flex justify-between items-start mb-2"><div className="flex items-center gap-1.5"><span className="text-[10px] font-black text-indigo-600">{h.date}</span><span className="text-[9px] font-black text-slate-500 bg-slate-200 px-1.5 py-0.5 rounded">{h.type}</span></div>{h.author && <span className="text-[8px] font-black text-slate-400 bg-white border border-slate-200 px-1.5 py-0.5 rounded-md flex items-center gap-1"><Edit2 size={8}/> {h.author}</span>}</div>
                               <div className={`overflow-hidden transition-all ${isExpanded ? 'max-h-[500px]' : 'max-h-[38px]'}`}><p className={`text-xs font-bold text-slate-700 leading-relaxed whitespace-pre-wrap ${!isExpanded ? 'line-clamp-2' : ''}`}>{h.note}</p></div>
                       ...
                       ) : (<p className="text-xs font-bold text-slate-500 text-center py-6">등록된 이력 메모가 없습니다.</p>)}
                     </div>
                   </div>
                   <GoogleAdPlaceholder type="banner" className="mt-6 mb-2" />
                 </>
                 ...
                 <div className="bg-white p-5 rounded-2xl border border-indigo-100 shadow-lg space-y-5 animate-in slide-in-from-bottom-4 duration-300">
                   ...
                   <div><textarea rows="5" placeholder="고객에 대한 중요한 메모나 특이사항을 기록해 주세요." value={newMemoText} onChange={e => setNewMemoText(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold text-slate-800 outline-none focus:border-indigo-400 resize-none leading-relaxed" autoFocus></textarea></div>
                   <div className="flex gap-2 pt-2 border-t border-slate-100">
                     <button onClick={() => setIsAddingMemo(false)} className="flex-1 bg-slate-100 text-slate-500 py-3.5 rounded-xl font-black text-xs transition-colors active:bg-slate-200">취소</button>
                     <button onClick={handleSaveMemo} className="flex-[2] bg-indigo-600 text-white py-3.5 rounded-xl font-black text-xs transition-transform active:scale-95 shadow-md shadow-indigo-200 flex items-center justify-center gap-1.5"><CheckCircle2 size={16} /> 메모 저장하기</button>
                   </div>
                 </div>
               )}
             </div>
  ...
  const renderBottomNav = () => (
    <div className="absolute bottom-0 inset-x-0 bg-white border-t border-slate-200 flex justify-between items-center h-[76px] pb-4 pt-1 px-6 z-40 rounded-b-[2.5rem]">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 -top-6 flex justify-center">
        <button onClick={handleOpenScanner} className="bg-indigo-600 text-white w-[60px] h-[60px] rounded-full shadow-lg shadow-indigo-200 flex flex-col items-center justify-center active:scale-95 transition-transform border-4 border-[#f8fafc]">
      ...
      <div className="flex gap-8">
        ...
        <button onClick={() => { setActiveTab('profile'); setSelectedCustomer(null); setSelectedTeamMember(null); }} className={`flex flex-col items-center gap-1 transition-colors ${activeTab === 'profile' && !selectedCustomer ? 'text-indigo-600' : 'text-slate-400'}`}>
          <div className="relative"><Bell size={22} /><span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-white"></span></div>
          ...
          <div className="relative w-full h-full bg-[#f8fafc] sm:rounded-[3rem] overflow-hidden flex flex-col font-sans select-none border border-slate-200/50">
            <div className="absolute top-0 inset-x-0 h-12 flex justify-between items-center px-7 z-[100] pointer-events-none text-slate-800">
              ...
              <div className="flex gap-1.5 items-center mt-1">
                {user ? <span className="text-[8px] bg-emerald-100 text-emerald-700 border border-emerald-200 px-1.5 py-0.5 rounded-md font-black mr-1 flex items-center gap-1"><Wifi size={8} className="animate-pulse"/> 서버 연결됨</span> : <span className="text-[8px] bg-slate-200 text-slate-500 px-1.5 py-0.5 rounded-md font-black mr-1">연결중...</span>}
              ...
              <>
                ...
                {isMyCardsModalOpen && (
                  <div className="absolute inset-0 z-[200] flex items-end justify-center animate-in fade-in duration-200">
                    ...
                    <div className="relative bg-slate-50 w-full rounded-t-[2.5rem] p-6 pb-8 animate-in slide-in-from-bottom duration-300 shadow-2xl max-h-[90%] flex flex-col">
                      ...
                      <div className="flex justify-between items-start mb-6 shrink-0">
                        ...
                        <button onClick={() => setIsMyCardsModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-white border border-slate-200 rounded-full text-slate-500 active:scale-95 transition-all"><X size={16} /></button>
                      </div>
                      <div className="flex-1 overflow-y-auto hide-scrollbar space-y-4 mb-6">
                        {myBusinessCards.map((card) => (
                          <div key={card.id} onClick={() => { setUserProfile(card); setIsMyCardsModalOpen(false); showToast("활성 프로필이 변경되었습니다."); }} className={`p-5 rounded-2xl border-2 cursor-pointer relative overflow-hidden flex flex-col justify-between aspect-[1.8/1] shadow-sm ${userProfile.id === card.id ? 'border-indigo-500 bg-gradient-to-br from-indigo-900 to-black text-white' : 'border-slate-200 bg-white hover:border-indigo-300 text-slate-900'}`}>
                            <div className="flex justify-between items-start relative z-10">
                              <div>
                                <p className={`text-[9px] font-black ${userProfile.id === card.id ? 'text-indigo-300' : 'text-slate-400'}`}>{card.company}</p>
                                <h3 className="text-xl font-black mt-1">{card.name}</h3>
                              </div>
                            </div>
                          </div>
                        ))}
                      ...
                      {myBusinessCards.length < 5 && (
                        <button onClick={handleAddNewProfileCard} className="shrink-0 w-full py-4 border-2 border-dashed border-indigo-300 bg-indigo-50/50 text-indigo-600 rounded-2xl font-black text-sm transition-all hover:bg-indigo-100 flex items-center justify-center gap-2 active:scale-95"><Plus size={18} /> 새 명함 등록하기</button>
                      )}
                    </div>
                  </div>
                )}
                ...
                {isMainScanning && (
                  <div className="absolute inset-0 z-[300] bg-slate-900 flex flex-col animate-in fade-in duration-300">
                    ...
                    {mainScanStep === 'choice' && (
                      <div className="flex-1 relative flex flex-col items-center justify-center p-6">
                         <div className="w-full flex flex-col gap-4 mt-4">
                           <button onClick={startMainScan} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-base shadow-lg shadow-indigo-900 active:scale-95 flex items-center justify-center gap-2"><Camera size={20} /> 명함 스캔하기</button>
                    ...
                    {mainScanStep === 'result' && (
                      <div className="flex-1 bg-slate-50 rounded-t-[2rem] flex flex-col mt-4">
                         ...
                         <div className="p-5 bg-white flex gap-2">
                           <button onClick={() => setMainScanStep('choice')} className="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-sm">뒤로</button>
                    ...
                    {mainScanStep === 'detail_choice' && (
                      <div className="flex-1 relative flex flex-col items-center justify-center p-6">
                         <div className="w-full flex flex-col gap-4 mt-4">
                           <button onClick={startUpdateScan} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black text-base flex items-center justify-center gap-2"><CreditCard size={20} /> 명함 교체 (업데이트)</button>
                           <button onClick={() => { setSelectedCustomer(null); setMainScanStep('choice'); }} className="w-full py-5 bg-slate-800 text-white border border-slate-700 rounded-2xl font-black text-base flex items-center justify-center gap-2"><UserPlus size={20} /> 전혀 다른 신규 고객 등록</button>
                         </div>
                      </div>
                    )}
                    ...
                    {mainScanStep === 'update_result' && (
                      <div className="flex-1 bg-slate-50 rounded-t-[2rem] flex flex-col mt-4">
                         <div className="p-6 flex-1 overflow-y-auto pb-32">
                           <p className="text-xs font-bold text-indigo-600 bg-indigo-50 p-4 rounded-xl border border-indigo-100 mb-4">이전 명함은 보관되며 최신으로 교체됩니다.</p>
                           <input value={scannedData?.name} onChange={e=>setScannedData({...scannedData, name: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold mb-3"/>
                         ...
                         <div className="p-5 bg-white flex gap-2">
                           <button onClick={() => setMainScanStep('detail_choice')} className="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-sm">뒤로</button>
                           <button onClick={handleSaveUpdatedCard} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm">업데이트 완료</button>
                         </div>
                      </div>
                    )}
                  </div>
                ...
                {isShareModalOpen && selectedCustomer && (
                  <div className="absolute inset-0 z-[200] flex items-end justify-center">
                    <div className="absolute inset-0 bg-slate-900/60" onClick={() => setIsShareModalOpen(false)} />
                    <div className="relative bg-white w-full rounded-t-[2.5rem] p-6 pb-8 shadow-2xl max-h-[90%] flex flex-col">
                      <div className="flex justify-between items-start mb-4">
                        <div><h3 className="text-xl font-black">고객 공유</h3></div>
                        <button onClick={() => setIsShareModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-slate-50 rounded-full"><X size={16} /></button>
                      </div>
                      <div className="flex-1 overflow-y-auto space-y-2 mb-6">
                        {TEAM_MEMBERS.map(member => (
                          <label key={member.id} className="flex items-center justify-between p-3 rounded-xl border-2 cursor-pointer border-slate-100 hover:bg-slate-50">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 font-black">{member.name[0]}</div>
                              <div><p className="font-black text-sm">{member.name}</p></div>
                            </div>
                      ...
                      <button onClick={() => { updateSelectedCustomer({ sharedWith: selectedShareMembers }); setIsShareModalOpen(false); showToast("공유가 완료되었습니다."); }} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-black">저장</button>
                    </div>
                  </div>
                )}

                {/* [신규 추가] 알림 상세 팝업 (공지/약속) */}
                {popupData && (
                  <div className="absolute inset-0 z-[350] flex items-center justify-center p-6 animate-in fade-in duration-200">
                    ...
                    <div className="relative bg-white w-full max-w-sm rounded-[2rem] p-6 shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col">
                      <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                          <div className={`w-8 h-8 rounded-full flex items-center justify-center ${popupData.data.bg} ${popupData.data.color}`}>
                            {React.createElement(popupData.data.icon, { size: 14, strokeWidth: 2.5 })}
                          </div>
                          <span className={`text-[10px] font-black px-2 py-1 rounded ${popupData.data.bg} ${popupData.data.color}`}>{popupData.data.type}</span>
                        </div>
                        <button onClick={() => setPopupData(null)} className="w-8 h-8 flex items-center justify-center bg-slate-50 rounded-full text-slate-500 active:scale-95 transition-all">
                          <X size={16} />
                        </button>
                      </div>
                      ...
                      {popupData.type === 'appointment' && (
                        <div className="space-y-3">
                          ...
                          <div className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl bg-white">
                            <div className="w-10 h-10 bg-slate-100 rounded-full flex items-center justify-center font-black text-slate-500">{popupData.data.payload.name[0]}</div>
                            <div className="flex-1">
                              <p className="text-sm font-black">{popupData.data.payload.name}</p>
                      ...
                      <button onClick={() => setPopupData(null)} className="w-full py-4 bg-slate-900 text-white rounded-xl font-black text-sm mt-6 active:scale-95 transition-all">확인했습니다</button>
                    </div>
                  </div>
                )}
              </>
