...
const GoogleAdPlaceholder = ({ type = 'banner', className = '' }) => {
  ...
  return (
    <div className={`bg-slate-200 border-2 border-dashed border-slate-300 rounded-2xl flex flex-col items-center justify-center text-center p-4 ${sizeClass} ${className}`}>
      <div className="w-8 h-8 rounded-full bg-slate-300 flex items-center justify-center mb-2">
...
export default function App() {
  ...
  const [userProfile, setUserProfile] = useState({ id: String(Date.now()), name: '', company: '', dept1: '', dept2: '', position: '', phone: '', email: '', experience: '', certs: [], isPublic: true });
  ...
  useEffect(() => {
    ...
    const unsubCustomers = onSnapshot(customersRef, async (snapshot) => {
      if (snapshot.empty) {
        const initialData = [
          { id: 1, name: "김태양", company: "미래IT", position: "수석", phone: "010-1234-5678", email: "sun@email.com", status: "가입", product: "종신/연금", lastContact: "2026-03-21", note: "상속세 관련 추가 상담 필요.", createdAt: "2026-01-10", hashtags: ['골프', '법인전환'], sharedWith: [1, 3], products: [{ id: 101, origin: '가입상품', category: '생명보험', company: '한화생명', productType: '종신/정기', name: '(무)한화생명 시그니처 종신보험', amount: '5억원', enrollDate: '2026-02', author: '본인' }], history: [{ id: 201, date: '2026-03-21', type: '대면상담', note: '상속세 관련 추가 상담 필요.', author: '본인' }] }
        ];
  ...
  const handleImageUpload = async (event) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        setScannedData({ name: name.replace(/[^가-힣a-zA-Z]/g, '').substring(0, 5), company: company, position: "직급/부서 입력", phone: phone, email: email, status: "가망", note: "Google AI 스캔으로 등록됨" });
  ...
  const handleSaveScannedData = async () => {
    ...
    const newCustomer = { id: String(Date.now()), ...scannedData, product: "미정", lastContact: currentDate, createdAt: currentDate, hashtags: [], sharedWith: [], products: [], history: [{ id: String(Date.now()), date: currentDate, type: '대면상담', note: 'Google AI 명함 스캔으로 신규 등록됨', author: userProfile.name || '본인' }] };
    try { await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', String(newCustomer.id)), newCustomer); setIsMainScanning(false); setScannedData(null); showToast(`${newCustomer.name} 고객이 등록되었습니다.`); setActiveTab('customers'); setCustomerTab(newCustomer.status); } catch(e) { console.error(e); }
  };
  ...
  const handleSaveManualData = async () => {
    ...
    try { await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'customers', String(newCustomer.id)), newCustomer); setIsMainScanning(false); setManualData({ name: '', phone: '', job: '회사원', company: '', dept: '', position: '' }); showToast(`가망 고객 등록 완료.`); setActiveTab('customers'); setCustomerTab('가망'); } catch(e) { console.error(e); }
  };
  ...
  const handleCompleteGeneralCard = async () => {
    ...
    try { await setDoc(doc(db, 'artifacts', appId, 'users', user.uid, 'businessCards', String(newCard.id)), newCard); setUserProfile(newCard); setAppState('main'); showToast("명함 등록 완료."); } catch(e) { console.error(e); }
  };
  ...
  const renderIntroPage = () => (
    <div className="absolute inset-0 z-[200] bg-slate-50 flex flex-col items-center justify-center animate-in fade-in duration-700">
      <div className="flex flex-col items-center gap-6 animate-in zoom-in-95 duration-1000">
        ...
        <div className="text-center space-y-2"><h1 className="text-4xl font-black text-slate-900 tracking-tighter">LeadUp <span className="text-indigo-600">Pro</span></h1></div>
      </div>
    </div>
  );
  ...
  const renderTermsPage = () => {
    ...
    return (
      <div className="absolute inset-0 z-[180] bg-slate-100 flex flex-col px-6 pt-20 pb-10 animate-in slide-in-from-right duration-300">
        ...
        <button disabled={!canProceed} onClick={() => setAppState('profile_step1')} className={`w-full py-5 rounded-2xl font-black text-base mt-6 shadow-md ${canProceed ? 'bg-indigo-600 text-white' : 'bg-slate-300 text-slate-500'}`}>다음으로</button>
      </div>
    );
  };
  ...
  const renderProfileStep1Page = () => (
    <div className="absolute inset-0 z-[160] bg-slate-100 flex flex-col animate-in slide-in-from-right duration-300">
      <div className="bg-white px-6 pt-14 pb-5 border-b border-slate-200 sticky top-0 z-10 shadow-sm flex items-center justify-between"><div><h2 className="text-2xl font-black text-slate-900">프로필 등록 <span className="text-indigo-600 ml-1">1/2</span></h2></div></div>
      <div className="flex-1 overflow-y-auto px-6 py-6 space-y-6 pb-32 hide-scrollbar">
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            <div className="col-span-2"><label className="text-[10px] font-bold text-slate-500 ml-1">소속 회사</label><input type="text" value={userProfile.company} onChange={e=>setUserProfile({...userProfile, company:e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mt-1 shadow-sm" /></div>
            <div><label className="text-[10px] font-bold text-slate-500 ml-1">직급</label><input type="text" value={userProfile.position} onChange={e=>setUserProfile({...userProfile, position:e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mt-1 shadow-sm" /></div>
            <div><label className="text-[10px] font-bold text-slate-500 ml-1">이름</label><input type="text" value={userProfile.name} onChange={e=>setUserProfile({...userProfile, name:e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mt-1 shadow-sm" /></div>
          </div>
        </div>
      </div>
      <div className="absolute bottom-0 inset-x-0 p-6"><button onClick={() => setAppState('profile_step2')} className="w-full py-4 bg-indigo-600 text-white rounded-2xl font-black text-base shadow-lg shadow-indigo-200">다음 단계로</button></div>
    </div>
  );
  ...
  const renderProfileStep2Page = () => {
    ...
    return (
      <div className="absolute inset-0 z-[160] bg-slate-100 flex flex-col animate-in slide-in-from-right duration-300">
        <div className="bg-white px-6 pt-14 pb-5 border-b border-slate-200 sticky top-0 z-10 shadow-sm flex items-center gap-3"><button onClick={() => setAppState('profile_step1')} className="w-8 h-8 flex items-center justify-center bg-slate-100 rounded-full text-slate-600 border border-slate-200 shadow-sm"><ChevronLeft size={18}/></button><div><h2 className="text-2xl font-black text-slate-900">프로필 등록 2/2</h2></div></div>
        <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8 pb-32 hide-scrollbar">
          <div className="flex flex-wrap gap-2">
            {CERTIFICATIONS.map(cert => {
              ...
              return <button key={cert} type="button" onClick={() => toggleCert(cert)} className={`px-4 py-2.5 rounded-xl text-xs font-black border shadow-sm transition-colors ${isActive ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50'}`}>{cert}</button>
            })}
          </div>
        </div>
        <div className="absolute bottom-0 inset-x-0 p-6"><button disabled={!isNextEnabled} onClick={handleCompleteGeneralCard} className={`w-full py-4 rounded-2xl font-black text-base shadow-lg ${isNextEnabled ? 'bg-slate-900 text-white shadow-slate-900/30' : 'bg-slate-300 text-slate-500'}`}>프로필 등록 완료</button></div>
      </div>
    );
  };
    ...
    return (
      <div className="p-5 space-y-5 pb-32 flex flex-col min-h-full">
        ...
        <section className="grid grid-cols-3 gap-3 mt-2">
          <button onClick={() => { setCustomerTab('가망'); setActiveTab('customers'); }} className="bg-white py-4 rounded-xl border border-slate-200 shadow-md flex flex-col items-center justify-center hover:border-blue-300 active:bg-slate-50">
            <UserPlus size={14} className="text-blue-500 mb-1.5" /><h4 className="text-[10px] font-black text-slate-700">가망 고객</h4><p className="text-sm font-black text-blue-600 mt-1">{customers.filter(c => c.status === '가망' && !c.isSharedWithMe).length}명</p>
          </button>
          <button onClick={() => { setCustomerTab('가입'); setActiveTab('customers'); }} className="bg-white py-4 rounded-xl border border-slate-200 shadow-md flex flex-col items-center justify-center hover:border-emerald-300 active:bg-slate-50">
            <UserCheck size={14} className="text-emerald-500 mb-1.5" /><h4 className="text-[10px] font-black text-slate-700">가입 고객</h4><p className="text-sm font-black text-emerald-600 mt-1">{customers.filter(c => c.status === '가입' && !c.isSharedWithMe).length}명</p>
          </button>
          <button onClick={() => { setCustomerTab('공유'); setActiveTab('customers'); }} className="bg-white py-4 rounded-xl border border-slate-200 shadow-md flex flex-col items-center justify-center hover:border-purple-300 active:bg-slate-50">
            <Share2 size={14} className="text-purple-500 mb-1.5" /><h4 className="text-[10px] font-black text-slate-700">공유 고객</h4><p className="text-sm font-black text-purple-600 mt-1">{customers.filter(c => c.isSharedWithMe).length}명</p>
          </button>
        </section>
        <section className="bg-white rounded-2xl border border-slate-200 p-5 shadow-md flex-1">
           <h3 className="text-xs font-black text-slate-900 mb-4">최근 연락처</h3>
           <div className="space-y-4">
            {customers.sort((a, b) => new Date(b.lastContact) - new Date(a.lastContact)).slice(0, 3).map(c => (
              <div key={c.id} onClick={() => { setSelectedCustomer(c); setCustomerDetailTab('기본정보'); }} className="flex items-center justify-between cursor-pointer group bg-slate-50/50 p-3 rounded-xl border border-slate-100 hover:border-indigo-200 transition-colors">
                <div className="flex items-center gap-3">
                  ...
                  <div><h4 className="font-black text-slate-900 text-sm">{c.name}</h4><p className="text-[10px] font-bold text-slate-500 mt-0.5">{c.company}</p></div>
                </div>
              </div>
            ))}
          </div>
  ...
  const renderCustomerListPage = () => (
    <div className="p-5 space-y-4 animate-in slide-in-from-right duration-300 pb-32">
      <div className="flex justify-between items-center pt-2 mb-2">
        ...
        <div className="bg-indigo-100 px-3 py-1.5 rounded-lg border border-indigo-200"><p className="text-[10px] font-black text-indigo-700">총 {filteredCustomers.length}명</p></div>
      </div>
      <div className="flex gap-1.5 p-1 bg-slate-200/60 rounded-xl mb-4 border border-slate-200/50">
        {['가망', '가입', '거래처', '공유'].map(tab => (
          <button key={tab} onClick={() => setCustomerTab(tab)} className={`flex-1 py-2.5 text-[12px] font-black rounded-lg transition-all ${customerTab === tab ? 'bg-white text-indigo-600 shadow-sm border border-slate-200/50' : 'text-slate-500'}`}>{tab}</button>
        ))}
      </div>
      <div className="grid grid-cols-1 gap-3">
        {filteredCustomers.length === 0 ? (
          <div className="py-16 text-center text-slate-500 font-bold text-sm">등록된 고객이 없습니다.</div>
        ) : (
          filteredCustomers.map((c, index) => (
            <React.Fragment key={c.id}>
              {index === 2 && <GoogleAdPlaceholder type="infeed" className="mb-3" />}
              <div onClick={() => { setSelectedCustomer(c); setCustomerDetailTab('기본정보'); }} className="bg-white rounded-2xl p-5 shadow-md border border-slate-200 flex items-center justify-between cursor-pointer hover:border-indigo-300 transition-colors">
                <div className="flex items-center gap-4 min-w-0">
                  <div className="w-12 h-12 bg-slate-50 border border-slate-200 rounded-2xl flex items-center justify-center text-slate-500 text-lg font-black shadow-sm">{c.name[0]}</div>
                  <div>
                    <h3 className="text-[15px] font-black text-slate-900 tracking-tight">{c.name}</h3>
                    <p className="text-[11px] font-bold text-slate-500 mt-1">{c.company}</p>
                  </div>
                </div>
              </div>
            </React.Fragment>
      ...
      <div className="p-5 space-y-5 pb-32">
        ...
        <div className="space-y-3">
           {displayMembers.map(member => (
             <div key={member.id} onClick={() => setSelectedTeamMember(member)} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-md flex items-center justify-between cursor-pointer hover:border-indigo-300">
               <div className="flex items-center gap-4">
                 <div className="w-12 h-12 bg-slate-50 border border-slate-200 text-slate-500 shadow-sm rounded-full flex items-center justify-center text-lg font-black">{member.name[0]}</div>
      ...
      <div className="p-5 space-y-5 pb-32">
        ...
        <div className="space-y-3">
          {notifications.map(noti => {
            ...
            return (
              <div key={noti.id} className="bg-white rounded-2xl p-4 shadow-md border border-slate-200 flex items-start gap-4">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border border-slate-100 shadow-sm ${noti.bg} ${noti.color}`}><Icon size={18} /></div>
  ...
  const renderTeamMemberDetailView = () => {
    ...
    return (
      <div className="pb-10 bg-slate-100 min-h-full">
        <div className="bg-white px-5 pt-4 pb-4 flex items-center sticky top-0 z-20 shadow-sm border-b border-slate-200 gap-3">
          <button onClick={() => setSelectedTeamMember(null)} className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 border border-slate-200 shadow-sm"><ChevronLeft size={20} /></button>
          <h2 className="text-lg font-black text-slate-900">동료 상세 프로필</h2>
        </div>
        <div className="p-5">
           <h3 className="text-2xl font-black mt-2 text-slate-900">{selectedTeamMember.name} <span className="text-sm">{selectedTeamMember.position}</span></h3>
        </div>
      </div>
    );
  };
    ...
    return (
      <div className="pb-10 bg-slate-100 min-h-full">
        <div className="bg-white px-5 pt-4 pb-4 flex justify-between items-center sticky top-0 z-20 shadow-sm border-b border-slate-200">
          <button onClick={() => setSelectedCustomer(null)} className="w-9 h-9 flex items-center justify-center rounded-full bg-slate-100 text-slate-600 border border-slate-200 shadow-sm active:scale-95 transition-all"><ChevronLeft size={20} /></button>
          <h2 className="text-lg font-black text-slate-900 tracking-tight">고객 상세 프로필</h2>
          {/* 💡 공유 팝업을 띄우는 공유 버튼 로직이 정상 작동하도록 확인 완료 */}
          <button onClick={() => { setShareSearchQuery(''); setSelectedShareMembers(selectedCustomer.sharedWith || []); setIsShareModalOpen(true); }} className="w-9 h-9 flex items-center justify-center rounded-full bg-indigo-100 text-indigo-600 border border-indigo-200 shadow-sm active:scale-95 transition-all"><Share2 size={16} strokeWidth={2.5} /></button>
        </div>
        <div className="p-5">
           <div className="bg-white rounded-3xl p-6 shadow-md border border-slate-200 relative overflow-hidden aspect-[1.6/1] flex flex-col justify-between">
              <div>
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">{selectedCustomer.company}</p>
                <h3 className="text-2xl font-black text-slate-900 mt-1">{selectedCustomer.name} <span className="text-sm font-bold text-slate-500 ml-1">{selectedCustomer.position}</span></h3>
              </div>
              <div>
                <p className="text-sm font-bold text-slate-600 flex items-center gap-2.5"><Phone size={14} className="text-indigo-400"/>{selectedCustomer.phone}</p>
              </div>
           </div>
        </div>
      </div>
  ...
  const renderBottomNav = () => (
    <div className="absolute bottom-0 inset-x-0 bg-white border-t border-slate-200 flex justify-between items-center h-[76px] pb-4 pt-1 px-6 z-40 rounded-b-[2.5rem]">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 -top-6 flex justify-center">
        <button onClick={handleOpenScanner} className="bg-indigo-600 text-white w-[60px] h-[60px] rounded-full shadow-lg shadow-indigo-300 flex flex-col items-center justify-center active:scale-95 border-[5px] border-slate-100">
              ...
              <>
                ...
                {isMainScanning && (
                  <div className="absolute inset-0 z-[300] bg-slate-900 flex flex-col animate-in fade-in duration-300">
                    ...
                    {mainScanStep === 'manual' && (
                      <div className="flex-1 bg-slate-100 rounded-t-[2rem] overflow-hidden flex flex-col mt-4 text-slate-800">
                         <div className="p-6 overflow-y-auto hide-scrollbar pb-32 flex-1 space-y-6">
                            ...
                            <div className="space-y-4">
                              <div><label className="text-[10px] font-bold text-slate-500 block mb-1.5 uppercase">고객명 *</label><input value={manualData.name} onChange={e=>setManualData({...manualData, name: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3.5 text-sm font-bold shadow-sm" /></div>
                              <div><label className="text-[10px] font-bold text-slate-500 block mb-1.5 uppercase">전화번호 *</label><input type="tel" value={manualData.phone} onChange={e=>setManualData({...manualData, phone: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3.5 text-sm font-bold shadow-sm" /></div>
                            </div>
                         </div>
                         <div className="p-5 bg-white border-t border-slate-200 pb-8 flex gap-2 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-10">
                           <button onClick={() => setMainScanStep('choice')} className="flex-1 py-4 bg-slate-100 text-slate-600 rounded-2xl font-black text-sm active:scale-95 shadow-sm border border-slate-200">뒤로</button>
                           <button onClick={handleSaveManualData} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm shadow-lg shadow-indigo-200 active:scale-95">고객 등록하기</button>
                         </div>
                      </div>
                    )}
                    {mainScanStep === 'camera' && (
                    ...
                    {mainScanStep === 'result' && (
                      <div className="flex-1 bg-slate-100 rounded-t-[2rem] flex flex-col mt-4">
                         <div className="p-6 flex-1 overflow-y-auto pb-32">
                            <h3 className="text-xl font-black mb-4">인식 결과 확인</h3>
                            <input value={scannedData?.name} onChange={e=>setScannedData({...scannedData, name: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mb-3 shadow-sm"/>
                            <input value={scannedData?.phone} onChange={e=>setScannedData({...scannedData, phone: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mb-3 shadow-sm"/>
                            <input value={scannedData?.company} onChange={e=>setScannedData({...scannedData, company: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mb-3 shadow-sm"/>
                            <input value={scannedData?.email} onChange={e=>setScannedData({...scannedData, email: e.target.value})} className="w-full bg-white border border-slate-300 rounded-xl px-4 py-3 text-sm font-bold mb-3 shadow-sm" placeholder="이메일"/>
                         </div>
                         <div className="p-5 bg-white flex gap-2 border-t border-slate-200">
                           <button onClick={() => setMainScanStep('choice')} className="flex-1 py-4 bg-slate-100 rounded-2xl font-black text-sm border border-slate-200 shadow-sm">다시 찍기</button>
                ...
                {/* 💡 공유 버튼 작동 불가 문제 해결: 모달 팝업의 z-index를 최상단(z-[999])으로 끌어올림 */}
                {isShareModalOpen && selectedCustomer && (
                  <div className="absolute inset-0 z-[999] flex items-end justify-center animate-in fade-in duration-200">
                    ...
                    <div className="relative bg-white w-full rounded-t-[2.5rem] p-6 pb-8 shadow-2xl max-h-[90%] flex flex-col animate-in slide-in-from-bottom-8 duration-300">
                      <div className="flex justify-between items-start mb-4">
                        ...
                        <button onClick={() => setIsShareModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-slate-100 border border-slate-200 shadow-sm rounded-full text-slate-600"><X size={16} /></button>
                      </div>
